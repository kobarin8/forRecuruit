...
//-------------------------------------------------------------------------------------------入力文字の判定
			...
			//String propertyName										警告文					文字
			//String value														警告対象の値			文字

			//int minLength													最小入力文字			数字
			//int maxLength													最大入力文字			数字

			//boolean availableAlphabeticCharacters			半角英字				true/false
			//boolean availableKanji										漢字						true/false
			//boolean availableHiragana								ひらがな				true/false
			//boolean availableHalfWidthDigit						半角数字				true/false
			//boolean availableHalfWidthSymbols					半角記号				true/false
			//boolean availableKatakana								カタカナ				true/false
			//boolean availableFullWidthSymbols					全角記号				true/false
	...
	//入力欄が空かどうかを検証します、リストの作成
					if
					(StringUtils.isEmpty(value))
					{stringList.add(propertyName + "を入力してください。");}

	//入力欄が最小文字数以上、最大文字数以下かどうかを検証します、リストの作成
					if
...
//--------------------------------------------------------------------------------------------boolean型の確認
	...
	//入力文字の
					if
					(availableAlphabeticCharacters || availableKanji || availableHiragana || availableHalfWidthDigit || availableHalfWidthSymbols||availableKatakana||availableFullWidthSymbols)
					{regularExpression = "[^";}
					if
					(!(availableAlphabeticCharacters) || !(availableKanji) || !(availableHiragana) || !(availableHalfWidthDigit) || !(availableHalfWidthSymbols)|| !(availableKatakana)|| !(availableFullWidthSymbols))
					{errorExpression = "[^";}
	...
	//
					if
					(!StringUtils.isEmpty(regularExpression))
					{regularExpression +="]+";}
					if
					(!StringUtils.isEmpty(errorExpression))
					{errorExpression +="]+";}
	...
	//----------------------------------------------------------------------------------------------------------------------
					...
					for
					(int i = 0;i < characterTypeList.size();i++)
					{
						if
						(i == 0)
	...
	//-----------------------------------------------------------------------------------------------------------------------リストの作成
					if
					(errorExpression.equals(""))
					{
						if
						(value.matches(regularExpression))
					...
					{
						if
						(value.matches(regularExpression)||(!value.matches(errorExpression)&&!value.equals("")))
						{stringList.add(propertyName + "は" + characterType + "で入力してください。");}
					}
	...
	//--------------------------------------------------------------------------------------------------------------------------
		...
		public List<String> doPasswordCheck(String password,String reConfirmationPassword){
			...
			if
			(!(password.equals(reConfirmationPassword)))
